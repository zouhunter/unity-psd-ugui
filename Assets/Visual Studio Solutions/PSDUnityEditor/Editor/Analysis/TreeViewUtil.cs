...
namespace PSDUnity
{
    ...
    {
        public static void TreeToList<T>(T root, IList<T> result,bool clear = false) where T : TreeViewItem
        {
            if (result == null)
        ...
        static bool IsChildOf<T>(T child, IList<T> elements) where T : TreeViewItem
        {
            while (child != null)
            {
